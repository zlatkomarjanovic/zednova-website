...
/** Transparent canvas — mark color only, for browser tab contrast. */
...
async function writePng(outPath, size, fill) {
  mkdirSync(dirname(outPath), { recursive: true });
  const buf = await sharp(Buffer.from(iconSvg(size, fill)))
    .png()
    .toBuffer();
...
async function main() {
  const lightMark = "#1F1F1F";
  const darkMark = "#F4F6F2";

  const outputs = [
    { path: "public/icon.png", size: 512, fill: lightMark },
    { path: "public/apple-icon.png", size: 180, fill: lightMark },
    { path: "public/apple-icon-dark.png", size: 180, fill: darkMark },
    { path: "public/favicon-16x16.png", size: 16, fill: lightMark },
    { path: "public/favicon-32x32.png", size: 32, fill: lightMark },
    { path: "public/favicon-32.png", size: 32, fill: lightMark },
    { path: "public/favicon-16x16-light.png", size: 16, fill: darkMark },
    { path: "public/favicon-32x32-light.png", size: 32, fill: darkMark },
  ];

  for (const { path, size, fill } of outputs) {
    await writePng(join(root, path), size, fill);
    console.log("wrote", path);
  }

  writeFileSync(
    join(root, "public/favicon.ico"),
    await sharp(Buffer.from(iconSvg(32, lightMark))).png().toBuffer(),
  );
